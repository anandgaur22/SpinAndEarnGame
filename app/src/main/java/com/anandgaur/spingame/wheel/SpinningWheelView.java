package com.anandgaur.spingame.wheel;
...
import com.anandgaur.spingame.R;
...
public class SpinningWheelView extends View implements WheelRotation.RotationListener {
    ...
    private void drawTriangle(Canvas canvas, Paint paint, float x, float y, float width, float height) {
        ...
        Drawable d =context.getResources().getDrawable(R.drawable.ic_arrow1);

        Bitmap bitmap= Bitmap.createBitmap(d.getIntrinsicWidth(),d.getIntrinsicHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas1 = new Canvas(bitmap);
        d.setBounds(0, 0, canvas1.getWidth(), canvas1.getHeight());
        d.draw(canvas1);
        returnedBitmap=convertToMutable(bitmap,height,width);
        ...
        canvas.drawBitmap(returnedBitmap,x - halfWidth, y - halfHeight,paint);
    }

    public static Bitmap convertToMutable(Bitmap imgIn, float height1, float width2) {
        try {
